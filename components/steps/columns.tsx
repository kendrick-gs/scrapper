...
import { columns, ProductRowData } from './columns';
...
function ProductTableView() {
  ...
  // FIX: Explicitly tell useReactTable which data type it's using
  const table = useReactTable<ProductRowData>({
    data: displayProducts,
    columns,
    state: { sorting, columnFilters, globalFilter },
    // A simpler, more direct way to define sub-rows
    getSubRows: (row) => (row as ShopifyProduct).variants,
  ...
  // FIX: Safely access properties on the original data for the memoized filters
      ...
      <div className="rounded-md border overflow-x-auto">
        <div className="bg-white relative min-w-[1500px]">
          ...
          <Table className="table-fixed w-full">
            <TableHeader>{table.getHeaderGroups().map(hg => (<TableRow key={hg.id}>{hg.headers.map(h => (<TableHead key={h.id} style={{ width: h.getSize() ? `${h.getSize()}px` : undefined }}>{flexRender(h.column.columnDef.header, h.getContext())}</TableHead>))}</TableRow>))}</TableHeader>
            <TableBody>{table.getRowModel().rows.map(row => (<TableRow key={row.id}>{row.getVisibleCells().map(cell => (<TableCell key={cell.id} style={{ width: cell.column.getSize() ? `${cell.column.getSize()}px` : undefined }} className="py-2 px-4 h-20">{flexRender(cell.column.columnDef.cell, cell.getContext())}</TableCell>))}</TableRow>))}</TableBody>
          </Table>
        </div>
      </div>
