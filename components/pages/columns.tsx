...
export const columns: ColumnDef<ProductRowData>[] = [
	...
	{ accessorKey:'body_html', header:'Body HTML', size:120, enableResizing:false, cell: ({row})=> { if(isVariant(row.original)) return null; const bodyHtml=(row.original as ShopifyProduct).body_html; if(!bodyHtml?.trim()) return null; return <Dialog><DialogTrigger asChild><Button variant="outline" size="sm">View</Button></DialogTrigger><DialogContent size="panel"><div className="px-5 pt-5 pb-4 border-b bg-gradient-to-b from-background to-background/70 sticky top-0 z-10"><DialogHeader className="px-0 py-0"><DialogTitle>{(row.original as ShopifyProduct).title}</DialogTitle></DialogHeader></div><div className="flex-1 overflow-y-auto p-5"><div className="prose dark:prose-invert max-w-none" dangerouslySetInnerHTML={{__html: bodyHtml}} /></div></DialogContent></Dialog>; } },
