...
export default function ListDetailPage(){
  ...
  const [tagsEditorOpen,setTagsEditorOpen]=useState(false); const [tagsEditorHandle,setTagsEditorHandle]=useState(''); const [tagsDraft,setTagsDraft]=useState<string[]>([]);
  ...
  const cols = source.map(col=>{
      ...
      if(col.accessorKey==='body_html'){
        return {
          ...col,
          header: ()=>(<div className="flex items-center justify-between w-full"><span>Body HTML</span><span className="absolute inset-y-0 right-0 w-px bg-border pointer-events-none" aria-hidden="true" /></div>),
          cell: ({row}: any) => {
  ...
  <Dialog open={tagsEditorOpen} onOpenChange={o=>{ setTagsEditorOpen(o); if(!o){ setTagsEditorHandle(''); } }}><DialogContent size="panel"><div className="px-5 pt-5 pb-4 border-b bg-gradient-to-b from-background to-background/70 sticky top-0 z-10"><DialogHeader className="px-0 py-0"><DialogTitle>Edit Tags - {tagsEditorHandle}</DialogTitle></DialogHeader></div><div className="flex-1 overflow-auto p-5 space-y-4 h-[70vh]"><div className="flex flex-wrap gap-2">{tagsDraft.map(t=> <span key={t} className="group flex items-center gap-1 bg-muted rounded px-2 py-1 text-xs">{t}<button className="opacity-60 group-hover:opacity-100" onClick={()=> setTagsDraft(d=> d.filter(x=>x!==t))}>×</button></span>)}<input className="h-8 border rounded px-2 text-xs w-40" placeholder="Add tag" onKeyDown={e=>{ if(e.key==='Enter'){ const v=(e.target as HTMLInputElement).value.trim(); if(!v) return; setTagsDraft(d=> Array.from(new Set([...d,v])).sort((a,b)=>a.localeCompare(b))); (e.target as HTMLInputElement).value=''; } }} /></div><div><div className="text-[11px] uppercase tracking-wide font-semibold mb-1 text-muted-foreground">Presets</div><div className="flex flex-wrap gap-1 max-h-48 overflow-auto pr-1">{dataPresets.tags.filter(t=> !tagsDraft.includes(t)).map(t=> <button key={t} onClick={()=> setTagsDraft(d=> Array.from(new Set([...d,t])).sort((a,b)=>a.localeCompare(b)))} className="text-xs border rounded px-1.5 py-0.5 hover:bg-primary hover:text-primary-foreground">{t} +</button>)}</div></div><div className="flex justify-end gap-2 pt-2"><Button variant="outline" size="sm" onClick={()=>setTagsEditorOpen(false)}>Cancel</Button><Button size="sm" onClick={()=>{ const val=tagsDraft.join(', '); applyLocalChange(tagsEditorHandle,{ tags:val }); let changed=false; const newSet=[...dataPresets.tags]; tagsDraft.forEach(t=>{ if(!newSet.includes(t)){ newSet.push(t); changed=true; } }); if(changed){ savePresets({ ...dataPresets, tags:newSet.sort((a,b)=>a.localeCompare(b)) }); } setTagsEditorOpen(false); }}>Save</Button></div></div></DialogContent></Dialog>
  </div>);
}
